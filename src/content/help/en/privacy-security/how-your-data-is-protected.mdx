...
<MockWindow title="Chiefy · Privacy" figcaption="Chiefy connects over OAuth and never stores your email password.">
  <SettingRow title="Connected over OAuth" iconKey="shield" description="Access via a revocable token — your password is never shared with Chiefy" />
  <SettingRow title="Encrypted at rest" iconKey="shield" description="Email contents and mailbox tokens are field-level encrypted before storage" />
</MockWindow>
...
{/* VERIFY: refresh-token replay detection + family-based revocation is verified in the codebase (token_families.py for product sessions; chatgpt_app/oauth_routes.py for the ChatGPT connector). This paragraph describes it at user altitude; confirm wording with the security team before publishing. */}
...
{/* VERIFY: this article intentionally avoids naming every encrypted collection/field and avoids hard compliance/certification claims (SOC 2, ISO, etc.). Confirm with the security team whether any certifications or additional guarantees should be stated, and whether the encryption-scope list above is complete and safe to publish. */}
